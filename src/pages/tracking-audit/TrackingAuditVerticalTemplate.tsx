...
/**
 * Parameterized copy of `src/pages/TrackingLandingPage.tsx` (the General
 * Tracking Audit page, owned by Kehinde + ChatGPT). This template exists so
 * vertical landing pages can reuse the approved application flow, styling,
 * anti-spam, attribution and tracking-event contracts without editing that
 * file. Only the hero headline/support copy, preset industry and SEO/canonical
 * fields differ per vertical — everything else (schema, options, sections,
 * FAQ, event names) is intentionally identical to the General page.
 */
...
export interface TrackingAuditVerticalTemplateProps {
  /** Industry value auto-set on step two; still shown as an editable Select, matching the General page's field behavior. */
  presetIndustry: PresetIndustry;
  heroHeadline: ReactNode;
  heroSupport: string;
  seoTitle: string;
  seoDescription: string;
  /** Canonical path for this vertical, e.g. "/offer/tracking-audit/professional-services". */
  canonicalUrl: string;
}

const TrackingAuditVerticalTemplate = ({
  presetIndustry,
  heroHeadline,
  heroSupport,
  seoTitle,
  seoDescription,
  canonicalUrl,
}: TrackingAuditVerticalTemplateProps) => {
  ...
  const {
    ...
    defaultValues: { adPlatforms: [], marketingOptIn: false, industry: presetIndustry },
  });
  ...
  const trackingAuditAnchorCta = {
    label: "Request a Free Tracking Audit",
    to: `${canonicalUrl}#claim`,
  } as const;
    ...
    <>
      ...
      <div className="border-t border-white/[0.06]">
        ...
        <CTASection
          ...
          description=""
          primaryCta={trackingAuditAnchorCta}
          secondaryCta={null}
...
export default TrackingAuditVerticalTemplate;
